...
#define OZ_LOGV(tag, format, ...) debugList.write(OZDEBUG_LOG_LEVEL_VERBOSE,tag,format, ##__VA_ARGS__)
#else
#define OZ_LOGV(tag, format, ...)
#endif

#if OZDEBUG_LOG_LEVEL >= OZDEBUG_LOG_LEVEL_DEBUG
#define OZ_LOGD(tag, format, ...) debugList.write(OZDEBUG_LOG_LEVEL_DEBUG,tag,format, ##__VA_ARGS__)
#else
#define OZ_LOGD(tag, format, ...)
#endif

#if OZDEBUG_LOG_LEVEL >= OZDEBUG_LOG_LEVEL_INFO
#define OZ_LOGI(tag, format, ...) debugList.write(OZDEBUG_LOG_LEVEL_INFO,tag,format, ##__VA_ARGS__)
#else
#define OZ_LOGI(tag, format, ...)
#endif

#if OZDEBUG_LOG_LEVEL >= OZDEBUG_LOG_LEVEL_WARN
#define OZ_LOGW(tag, format, ...) debugList.write(OZDEBUG_LOG_LEVEL_WARN,tag,format, ##__VA_ARGS__)
#else
#define OZ_LOGW(tag, format, ...)
#endif

#if OZDEBUG_LOG_LEVEL >= OZDEBUG_LOG_LEVEL_ERROR
#define OZ_LOGE(tag, format, ...) debugList.write(OZDEBUG_LOG_LEVEL_ERROR,tag,format, ##__VA_ARGS__)
#else
